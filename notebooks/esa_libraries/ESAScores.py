...
from esa_libraries.affiliation import pr_from_events
from esa_libraries.utils import convert_time_series_to_events, NANOSECONDS_IN_SECOND
...
class ESAScores():
    ...
    def score(self, y_true: pd.DataFrame, y_pred: np.ndarray) -> dict:
        ...
        if len(events_pred_ns) > 0:
            corrected_full_upper_range = max(np.max(events_pred_ns), np.max(events_gt_ns), self.full_range[1].value)
        else:
            corrected_full_upper_range = max(np.max(events_gt_ns), self.full_range[1].value)
